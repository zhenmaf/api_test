...
class Base():
    ...
    """
    返回一个完整的url
    param path : 接口路径，如： /admin/adminn/update
    :param params:查询擦书，如：?page=1&limit=20&sort=add_time&order=desc
    ...
    def login(self):
        """
        通过调用登录接口获取token值，将其进行缓存，其他接口使用时直接从缓存中取值。
        若没有取到，再调用登录。在将token值放入缓存中。
        :return:
        """
...
if __name__ == '__main__':
    ...
    # print(base.get_url('/admin/admin/update','?page=1&limit=20&sort=add_time&order=desc'))
